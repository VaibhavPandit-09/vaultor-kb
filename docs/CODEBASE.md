...
Opening a note appends or replaces panes according to settings. Pane count is bounded 1–3; closing a pane is distinct from deleting a resource. File previews do not consume a pane. Library filtering combines collection, type and all selected tags. Collections is a visible destination with search and New collection. Note headers and file previews show two membership chips plus overflow and a shared Add to collection picker. The same picker serves Library rows and bulk selections of up to 100 resources, with per-toggle pending/error/rollback/retry. Inline creation uses a stable UUID and atomically adds selected members. Collection headers offer Pin, New note, Add existing, Import files and secondary rename/delete; deletion keeps resources. Creation/import inside the visible collection captures that destination. Pinned combines notes, files and collections, alphabetically ordered, with 12 shortcuts and searchable paged View all. Pin controls appear at each resource and collection. Existing favorite fields remain authoritative; no reset or migration is needed. See [LIBRARY.md](LIBRARY.md) for exact semantics. Ctrl+K defaults to saved-note/title search with deduplicated Open/Pinned/Recent items and separate collection results. Commands mode uses > or a visible button. Actions name the selected resource; captured focus/Library selection determines command targets, never hidden notes. Errors retain input, in-flight actions cannot repeat, Tab remains focus navigation and Escape returns one step. Files preview only on explicit opening. See SEARCH.md for limits, index lifecycle and diagnostics.
...
- Dashboard: `h-screen`, hidden outer overflow, scrollable internal regions. Sidebar contains Search, Library, Recent, Collections and Pinned navigation, bounded quick-access groups, New collection/New Note/Import files, tag chips/color controls, and footer export/import/help/settings/theme/diagnostics controls. Library provides saved-note/title search with safe excerpts, type/sort controls, counts, paging, membership actions and pin icons using shared theme surfaces. Resource membership chips wrap in narrow panes; the shared picker uses the existing modal/Escape surface portaled to the document body.
