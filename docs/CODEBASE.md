...
Last reviewed: 2026-09-24. This is the current implementation reference. Update it in every change as required by [AGENTS.md](../AGENTS.md). Historical specifications are context only. See [API.md](API.md), [OPERATIONS.md](OPERATIONS.md), and the [documentation index](README.md).

## Product and architecture

Vaultor is a personal, locally hosted knowledge workspace. Notes and uploaded files are resources, organized by flat collections and tags and connected through inline links. The UI opens directly into the workspace. There are no accounts, authentication, master passwords, tokens, lock controls, encryption, or encrypted archive compatibility. Obsolete credentials stored in an existing database are ignored; startup does not delete resources.
...
| Palette rendering, explicit actions and saved search | `frontend/src/components/modals/CommandPaletteModal.tsx` |
| Saved search/index/diagnostics | `ResourceSearchService.java`, `SearchDiagnosticsController.java`, `lib/resourceSearch.ts`, `components/SearchExcerpt.tsx`; [SEARCH.md](SEARCH.md) |
...
| Resource organization and pins | `ResourceCollections.tsx`, `PinButton.tsx`, `OrganizationViews.tsx`, `CollectionManagement.tsx`, `AddExistingResources.tsx`, `lib/organizationControls.ts` |
...
| Entity | Contract |
| --- | --- |
| Resource / resources | UUID string id, type note/file, title, serialized content, filePath, MIME, size, createdAt/updatedAt/lastOpenedAt, favorite, many-to-many tags and collections |
| ResourceCollection / collections | UUID id, display name, unique normalized_name, favorite, internal creationFingerprint for retry-safe creation; resource_collections joins resources to multiple collections |
...
| resource_fts | Derived SQLite FTS5 resource titles/visible saved-note text; resource triggers maintain transactional consistency; startup/Diagnostics rebuild in gated batches |
| TransferOperation | persisted kind, status, phase, progress, mode, detail, requestId, timestamps, cleanup journal |

SQLite uses Hibernate ddl-auto=update with hibernate.hbm2ddl.jdbc_metadata_extraction_strategy=individually; there is no versioned migration framework. Individual inspection is required because existing SQLite FTS5/shadow tables expose untyped columns that crash Hibernate grouped metadata extraction on restart. Keep schema updates enabled and leave the derived search tables/data intact. Resource DTOs omit internal file paths and expose note content as a JSON object (never a JSON-encoded string). Entity persistence still stores content as text. Dates use LocalDateTime without a timezone suffix. Opening a resource updates only lastOpenedAt through a metadata SQL write; it does not change updatedAt or hydrate/save its document. Some tag mutations can still change updatedAt, so it is not strictly a content timestamp. Import preserves archived IDs on replacement and allocates new IDs on merge.
...
Use [API.md](API.md) for flows and /api/openapi.json for the machine contract. The same APIs serve agents and UI, without a privileged execution interface. Explicit DTOs cover resources, tags, settings and operations. List responses include items/page/size/totalItems/totalPages (size 1–200). Browsing returns metadata-only summaries with title/collection/type/all-selected-tags/favorites filters and recent/updated/title sorting. GET /resources/query supplies relevance-ranked saved-note/title search with metadata and safe snippets (size 1–100); legacy /resources/search still caps title matches at 50. See SEARCH.md for search/index contracts. GET resource detail supplies document content. Resource details and list summaries include collections [{id,name}]; list enrichment batches tags and collections without reading documents. PUT /resources/{id}/favorite persists a boolean without changing modification time. GET /organization/pins pages mixed shortcuts; POST /organization/selection reads membership counts for selected IDs. PUT /collections/creations/{id} creates once with initial members, and optional collectionId on note creation/file imports assigns membership transactionally. See [Library workflows](LIBRARY.md).
...
RequestLoggingFilter takes a shared workspace gate for resource/tag/settings/collection/organization calls. Transfers take the exclusive gate during snapshot/commit and search rebuild takes it per batch of 100, and concurrent calls receive 409 WORKSPACE_BUSY. The gate is process-local: run a single backend against a data directory. Health reports readiness and busy state; integrity checks never repair data.

## Frontend state and workflows

Design rule: **Simplicity is sophisticated**. Expose primary actions, group secondary actions, and preserve discoverability and basic keyboard access. Collection/create/rename forms use native submission with shared composition guards (`lib/useFormKeyboard.ts`); Enter and clicking submit run the same handler. Synchronous in-flight guards prevent repeat submission; pending name inputs remain read-only/focused and failed values are retained. Collection creation sends the current trimmed name directly to the existing retry-safe server contract, independently of debounced suggestion results; the server resolves uniqueness and uncertain-response retries. Non-submit buttons inside forms explicitly use type=button.

Root composition is StrictMode, Redux, application ErrorBoundary, SettingsProvider and EscapeManagerProvider. App mounts Dashboard directly. Redux owns resource navigation/history and filters. Dashboard owns resource list/detail caches, open panes, previews and modals. Tiptap owns live documents/selections. Settings context owns the normalized configuration. The API client converts legacy internal string content into JSON at the HTTP boundary; resourceBrowse/useResourcePage fetch bounded metadata pages through useRetainedQuery. Frontend SearchMode (title | content) joins the retained query key and is omitted from HTTP parameters: title uses GET /resources; nonempty content uses GET /resources/query. Scope/mode changes invalidate stale data, and only content mode renders body excerpts. Same-query background refreshes retain mounted rows; changed queries and workspace replacement clear stale results. Sidebar Recent and mixed Pinned shortcuts each show at most 12 entries; the Library virtualizes 100-entry server pages. Note editors remain mounted while Library is visible, with active focus and close-note shortcut disabled. Detail fetching is independent of the quick-access lists.

Opening a note appends or replaces panes according to settings. Pane count is bounded 1–3; closing a pane is distinct from deleting a resource. File previews do not consume a pane. Library filtering combines collection, type and all selected tags. Collections is a visible destination with search and New collection. Note headers and file previews show two membership chips plus overflow and a shared Add to collection picker. The same picker serves Library rows and bulk selections of up to 100 resources, with per-toggle pending/error/rollback/retry. Inline creation uses a stable UUID and atomically adds selected members. Collection headers offer Pin and Add resource; More holds rename/delete and management, and deletion keeps resources. Add resource searches titles, immediately adds existing notes/files with per-row feedback, creates notes with atomic initial membership, and hands off imports with the captured destination. Secondary creation permits duplicate titles. Library uses breadcrumbs, compact Search options (type/sort), removable filter chips, and selection-only bulk controls. Creation/import inside the visible collection captures that destination. Pinned combines notes, files and collections, alphabetically ordered, with 12 shortcuts and searchable paged View all. Pin controls appear at each resource and collection. Existing favorite fields remain authoritative; no reset or migration is needed. See [LIBRARY.md](LIBRARY.md) for exact semantics. Ctrl+K defaults to title-only search and Entire workspace on each opening, with deduplicated Open/Pinned/Recent items. Search options enables saved note text and collection scope; global title mode includes collection-name matches, while scoped mode lists only member resources. Library starts title-only and uses its current collection, with query-preserving scope changes updating the destination/breadcrumb. Commands mode uses > or a visible button. Actions name the selected resource; captured focus/Library selection determines command targets, never hidden notes. Errors retain input, in-flight actions cannot repeat, Tab remains native focus navigation: Actions follows the palette input in DOM order while CSS retains its footer position. Tab focuses Actions, Enter/Space activates it, and Shift+Tab returns to input. Escape returns one step with the previous highlighted resource/query restored. Files preview only on explicit opening. See SEARCH.md for limits, index lifecycle and diagnostics.
...
Application and independent editor/settings/preview/transfer/diagnostics boundaries show recovery UI and copyable diagnostic details. Regional recovery preserves sibling editor state. Action/network failures normally show a banner. Background browsing and recency failures retain diagnostics/request IDs but use local retry feedback, avoiding a global banner during navigation. Settings writes are serialized with visible failure/retry. Preview requests abort/ignore stale results and clean up object URLs. Diagnostic delivery failures never trigger further diagnostic requests or break UI: queue 100, batch 20, three delivery attempts, local history 100; backend history 200 events/24 hours/process lifetime.
...
Tables use WorkspaceTable.ts, TableControls.tsx and tableCommands.ts in the editor directory. In-note editing of the selected table in the active pane has a compact floating toolbar: Row, Column, Filter, Expand and More. It is a portal outside note layout, aligned above the table's right edge when space allows, otherwise following its visible upper edge within its own pane. It disappears offscreen; narrow panes use icon-only buttons with accessible hover labels. Repeated row/column edge buttons are removed in both views. Row/Column actions target the clicked cell/current range; opening menus preserves selection. Explicit Select row/Select column actions remain available, with affected indices shown in menu headings. Row/column menus provide structural operations, headers and sizing; More contains merge/split, copy/export, undo/redo, help and confirmed whole-table deletion. Filter and copy/export have separate popovers with local feedback. Destructive/structural restrictions while filtered remain unchanged. Menus preserve editor selection and dismiss on outside click or shared Escape. Filter-driven selection movement does not dismiss an active filter input.

Table chrome uses opaque surface-3, border-strong and elevated shadows instead of document styling. tableOverlayGeometry.ts intersects viewport and clipping ancestors, stopping at the fullscreen top layer. Geometry updates on transactions, captured scrolling and ancestor resizing; the shared positioning hook accepts optional pane bounds for popovers while preserving existing unbounded callers. Fullscreen popovers attach inside the existing editor container. Expanded editing uses a separate non-scrolling header and grouped editing bar, not the floating toolbar. The header shows note title, dimensions, live save state/retry and Return to note. A collapsible filter panel is docked below the bar. The existing EditorContent is a flexing scroll canvas; row/column handles are clipped to it so they cannot overlap the header. Small screens wrap action groups and bound chrome/filter height to retain canvas space. Entry preserves stored widths; Fit width is explicit. BlockEditor receives noteTitle/saveStatus/onRetrySave from Dashboard, including them in its memo comparison. This reuses the same editor, coordinator and undo history. Drag selection/resizing still use ProseMirror. Moves require one row/column and no spans; whole-table axis deletion is disabled in favor of Delete table.
...
- Dashboard: `h-screen`, hidden outer overflow, scrollable internal regions. Sidebar contains Search, Library, Recent, Collections and Pinned navigation, bounded quick-access groups, New collection/New Note/Import files, tag chips/color controls, and footer export/import/help/settings/theme/diagnostics controls. Library provides default title search with optional saved-note excerpts, compact Search options (content opt-in, collection scope, type/sort), breadcrumbs, counts, paging, membership actions and pin icons using shared theme surfaces. More contains management/refresh; Add resource replaces separate collection create/add/import buttons. Resource membership chips wrap in narrow panes; the shared picker uses the existing modal/Escape surface portaled to the document body.
...
- Command palette is a custom overlay with a panel capped at 42rem, search-first resource/collection groups, explicit Actions, Commands mode and multi-step interaction. Selected rows and snippet marks use theme tokens. Generic dialogs use `AppModal`, default `max-w-md`, rounded 28px shell, header/body/footer.
...
ZIP version 2 contains manifest.json (version, creation time, counts, SHA-256 checksums), workspace.json (logical resources/tags/workspace settings plus organization: collections, memberships and resource/collection favorites), and files/{resource-id}. No live SQLite file, credentials, diagnostics or device appearance/keybindings are exported. JSON entries cap at 20 MiB, archive defaults to 512 MiB, expanded data 2 GiB and 20,000 entries. Configure limits through environment variables; malformed paths, duplicate entries, checksums, counts and document/reference shapes are validated before mutation. Missing document targets appear as preview warnings.

Preview stages the upload. Merge generates a new resource graph and rewrites both inline links and table source IDs; same-name tags reuse existing colors and workspace settings stay unchanged. Replace requires explicit confirmation and replaces resources/tags/files/workspace settings while retaining local appearance and keybindings. Both derive backlinks from documents. Merge remaps collection members to newly allocated resource IDs, unifies collections by normalized name and preserves existing collection favorites. Replace restores archived collection IDs and all favorites. Organization validates before mutation and restores within the metadata transaction. Plain version 1 archives remain readable without organization. Repeating a commit returns the original operation, never applies twice; to retry a failed commit upload/preview again.
...
- 2026-09-23 Workspace evolution Sprint 2: dedicated expanded-table header/action bar, docked filters, independently scrolling editor canvas and live note save status/retry. Same editor and stored widths/history preserved; no API/data changes. Frontend build and targeted lint pass; six table-control checks and three editor regressions pass. One disposable light-theme fullscreen visual check confirmed header separation; dark/small-window appearance not manually rechecked per the minimal-browser preference. No API/Docker work. Sprint 3 remains unstarted.

### 2026-09-23 — Workspace evolution Sprint 3

Implemented the Library and bounded sidebar navigation, persistent resource favorites, server-filtered metadata pages and bounded palette search. Added resource presentation registry; hidden note editors retain their state. API/schema change: nullable favorite column and resource indexes; browse DTOs omit bodies. Details and limitations: [LIBRARY.md](LIBRARY.md). Frontend production build, targeted ESLint, three component/hook tests and one disposable SQLite integration test (205 resources) pass. No browser/live API/Docker verification; deployment unchanged. Table handles unchanged pending discussion. Sprint 4 not started.

### 2026-09-23 — Workspace evolution Sprint 4

Added flat collections, collection favorites, paginated collection/tag management and atomic page-scoped bulk memberships. Schema adds collections/resource_collections; shared mutation gate includes organization routes. Version 2 workspace archives preserve/remap organization and favorites within the import transaction; version 1 plain archives remain readable. Removed repeated table edge buttons in both views; Row/Column actions target current cell/selection and preserve selection when opened. See LIBRARY.md, API.md and TABLES.md. One focused disposable SQLite integration test and 15 frontend cases pass; frontend build and targeted lint pass. No browser, live API or Docker verification; deployment unchanged. Sprint 5 not started.

### 2026-09-23 — Usability Sprint U1

Fixed navigation-triggered sidebar loading flashes. Typed resource-change notifications distinguish opened/metadata/organization/pins/workspace changes. RecencyRecorder coalesces activation bursts for 300 ms, serializes writes and exposes local retry; only Recent reconciles 150 ms after the final write settles. Unrelated groups remain untouched. Requests abort or disregard stale responses; same-query rows and sidebar scroll remain stable through refresh/failure. Backend mark-open updates only lastOpenedAt. Details: LIBRARY.md. Production frontend build and targeted lint pass; 12 focused frontend cases (5 navigation/diagnostics, 4 Library, 3 organization manager) and one disposable backend browsing/timestamp integration test pass. No browser, live API smoke or Docker work; visual/native behavior remains unverified and deployment unchanged. U2 and U3 are authorized roadmap entries but not implemented; original full-text Sprint 5 is absorbed into U3.

### 2026-09-23 — U2: organization at the resource and unified pins

Implemented resource membership chips, the shared collection picker, collection destinations/actions and a mixed Pinned section. Added batched membership summaries, metadata selection/pin queries, retry-safe atomic collection creation, and transactional initial note/import membership. Schema adds only internal collection creation_fingerprint; existing favorite fields and archive v2 are preserved. Updated API/OpenAPI and Library documentation. Shared focus restoration handles picker return without replacing drafts. Frontend production build and targeted ESLint pass. Eighteen distinct focused frontend cases pass (membership rollback/retry and stable creation IDs, pin synchronization, Library paging/filtering, navigation refresh stability, import destination/retries and organization management). Two disposable SQLite integration cases pass, covering atomic creation/import membership, retry fingerprints, mixed pin ordering, metadata summaries, and archive organization merge/replace/rollback. One isolated browser fixture checked the membership header/pinned list in both themes, a narrow note pane and picker/Escape dismissal; it was removed afterward. No broad API smoke or Docker work. Integrated Dashboard/file-preview layout has not had a full manual pass; deployment remains unchanged. Existing bundle-size/Browserslist warnings remain. U3 remains unimplemented.

### 2026-09-24 — U3: search-first palette and saved-note search

Rebuilt Ctrl+K around deduplicated Open/Pinned/Recent and grouped saved-note/title/collection matches, explicit Actions and Commands mode, captured targets, retained failure input, normal Tab and layered Escape. Removed highlight-driven previews. Added derived SQLite FTS5 with transaction-local triggers, literal prefix ranking/snippets, shared Library search and bounded coordinated rebuild/status diagnostics. New API/OpenAPI contracts and authoritative SEARCH.md document index behavior and limitations. Frontend production build and targeted ESLint pass; 13 focused frontend cases and two disposable SQLite integration cases pass, including archive search consistency. OpenAPI JSON/operation IDs and diff whitespace validated. Isolated light/dark palette visual checks verified grouping, snippet/selection contrast and Escape; fixture/server removed. No broad API smoke or Docker rebuild. Full integrated Dashboard/assistive-technology/scale verification and file-content search remain future work. Existing build warnings remain; deployment unchanged. U3 complete; stop for discussion.

### 2026-09-24 — K1: basic form and palette keyboard behavior

Collection creation now submits on Enter without waiting for debounced search. Current-name/server validation and existing creation UUID retries remain authoritative. Added shared composition-safe form handling, synchronous duplicate guards, initial/rename focus and explicit focus styles; audited collection/tag manager, collection rename and note-title editing. Library organization dialogs and palette organization handoffs restore focus on dismissal. Palette Actions follows its input in native tab order; step back restores result identity/query. No API/schema changes. Production frontend build passes; final TypeScript compilation, 16 focused component cases and targeted ESLint pass. No browser, live API or Docker checks; native/visual verification not claimed. Existing bundle-size/Browserslist warnings remain. K2/K3 are unstarted.

### 2026-09-24 — K2: unified collection add and compact Library controls

Replaced collection New note/Add existing/Import buttons with Add resource and secondary More. The picker adds existing resources immediately, keeps pending/error/Added feedback per resource, searches titles, supports inline and explicit duplicate-title note creation, and imports into the captured collection. Creation retries retain UUID/destination; opening a created note has a separate promise-returning callback so save/open failures stay retryable without changing ordinary Library navigation. Search options reuse anchored positioning and Escape layers; breadcrumbs replace Clear collection and bulk controls appear only with selection. No backend/API/schema change. Frontend production build, final TypeScript and targeted ESLint pass; nine focused tests cover Library/picker/popover behavior. Isolated visual check covered dark wide collection layout, light narrow layout, picker and options placement; fixture/server removed. No broad API smoke or Docker work. Exact-name detection currently scans matching metadata in bounded pages; very broad title queries may take additional requests. Title-first global/Library search was implemented in the subsequent K3 change.

### 2026-09-24 — K3: title-first search and explicit scope

Implemented title-only defaults in Library/Ctrl+K, shared searchable collection/global scope controls and saved-text opt-in. Query text and open drafts are preserved; scope changes update Library destination/breadcrumb, reset pagination and discard stale requests. Scoped palette empty results exclude global suggestions. Palette reopening resets global/title mode; nested options use higher Escape priority and stop portal-click propagation. No backend/API/schema change; existing title/FTS endpoints retain their contracts. Production frontend build, final TypeScript and targeted ESLint pass; 15 distinct focused tests cover query routing, stale mode/scope results, palette reset/isolation, Library query retention and popover dismissal. No new browser/API/Docker checks; K2 supplied the narrow layout review, and the new scope controls are component-verified rather than visually verified. Existing bundle/Browserslist warnings remain. K1–K3 complete; stop for discussion.

### 2026-09-24 — Container restart with persisted search tables

Fixed Hibernate startup NoSuchElementException in columnInformation by switching JDBC metadata extraction from grouped to individually. Grouped discovery parsed untyped SQLite FTS5/shadow columns before the search service initialized. Added ExistingSearchIndexStartupTest, seeding an FTS index before application startup: reproduced the same failure before the fix and passed afterward, including a saved-text search assertion. Docker image rebuilt and normal Compose container started and restarted successfully on its unchanged named volume. Health returned UP/ready, search coverage complete (5 resources/5 indexed, no invalid documents), and restart count was zero. No workspace reset, resource edits, volume deletion or browser checks.
