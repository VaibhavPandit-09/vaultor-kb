...
| ResourceCollection / collections | UUID id, display name, unique normalized_name, favorite; resource_collections joins resources to multiple collections |
...
- Dashboard: `h-screen`, hidden outer overflow, scrollable internal regions. Sidebar contains Search, Library, Recent and Favorites navigation, bounded quick-access groups, New Note/Import files, tag chips/color controls, and footer export/import/help/settings/theme/diagnostics controls. Library provides title search, type/sort controls, counts, paging and favorite stars using shared theme surfaces.
