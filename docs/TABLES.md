...
The toolbar is outside document flow and uses a raised, opaque theme surface, border and shadow. It sits above the table's right edge where space permits, otherwise at its visible upper edge within the owning pane. It follows scrolling/resizing and hides when the table is offscreen or no longer selected. Narrow panes show icons with hover labels. Row/column handles open action popovers beside the chosen handle; selection always determines the edit target.
...
- `TableControls.tsx`: floating portal toolbar, handle-anchored structural popovers, Escape/focus and same-container fullscreen lifecycle.
