...
export default function LibraryPopover({label,children,priority=ESCAPE_PRIORITIES.popover}:{label:string;priority?:number;children:(close:()=>void)=>ReactNode}) {
 ...
 useEscapeLayer({active:open,priority,close,restoreFocusOnEscape:false});
 ...
 return <><button ref={anchor} className="library-button" aria-expanded={open} aria-haspopup="dialog" onClick={()=>setOpen(v=>!v)}>{label}</button>{open&&position&&createPortal(<div ref={panel} onClick={e=>e.stopPropagation()} role="dialog" aria-label={label} className="library-popover" style={{position:'fixed',zIndex:85,top:position.top,left:position.left,width:position.width,maxHeight:position.maxHeight,transform:position.placement==='top'?'translateY(-100%)':undefined}}>{children(close)}</div>,document.body)}</>;
}
