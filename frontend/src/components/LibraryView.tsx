...
export default function LibraryView({ section, visible, tags, onOpen, onReturn, hasNotes, collection = null, onCollection = () => {}, onTagsChange = () => {}, onNewNote, onImport, onSelectionChange, hasUnsavedChanges=false }: { onSelectionChange?:(items:Resource[])=>void; hasUnsavedChanges?:boolean; onNewNote?:()=>void; onImport?:()=>void; collection?: OrganizationItem | null; onCollection?: (item: OrganizationItem | null) => void; onTagsChange?: (tags: string[]) => void; section: LibrarySection; visible: boolean; tags: string[]; onOpen: (id: string) => void; onReturn: () => void; hasNotes: boolean }) {
  ...
  return <section className="library-view" aria-label={title}>
    ...
    <div className="library-filters">
      ...
      <label className="library-search"><Search size={17} /><input aria-label="Search saved resources" placeholder="Search titles and saved notes…" value={query} onChange={event => setQuery(event.target.value)} /></label>
      ...
      <select aria-label="Sort resources" value={section === 'recent' ? 'recent' : sort} disabled={section === 'recent'||Boolean(search)} onChange={event => { setSort(event.target.value as typeof sort); setPage(0); }}><option value="updated">Recently updated</option><option value="recent">Recently opened</option><option value="title">Title A–Z</option></select>
      <button className="library-button" aria-label="Refresh Library" title="Refresh" onClick={retry}><RefreshCw size={16} /></button>
    </div>
    <div className="library-summary">{loading ? 'Loading resources…' : `${data?.totalItems ?? 0} resources`}{tags.length > 0 && <span> · Matching tags: {tags.join(', ')}</span>}<span className="ml-auto">{hasUnsavedChanges?'Search uses saved content · unsaved edits excluded':'Saved note text and file titles'}</span></div>
