...
export default function CommandPaletteModal({open,onClose,context}:Props) {
 ...
 const [searchMode,setSearchMode]=useState<SearchMode>('title'),[searchCollection,setSearchCollection]=useState<OrganizationItem|null>(null);
 ...
 if(open!==wasOpen){setWasOpen(open);if(open){setCaptured(context);setSearchMode('title');setSearchCollection(null);setQuery('');setSearch('');setMode('search');setStack([]);setSelectedId('');setError('');}}
 ...
 const resources=useResourcePage({q:search,size:50,sort:search.trim()?'title':'recent',searchMode,collection:searchCollection?.id},open&&((mode==='search'&&!entry)||selecting));
 const collections=useOrganizationPage('collection',search,0,open&&mode==='search'&&!entry&&!searchCollection&&searchMode==='title'&&Boolean(search.trim()),false,20);
 const pins=usePins('',0,30,open&&mode==='search'&&!search.trim()&&!entry&&!searchCollection);
 const scope={...captured,resources:resources.data?.items??[]};
 function close(){if(running.current)return;if(stack.length){setSelectedId(stack.at(-1)!.parentSelectedId);setStack(v=>v.slice(0,-1));setError('');}else if(mode==='commands'){setMode('search');setQuery('');setError('');}else onClose();}
 ...
 else if(search.trim()||searchCollection)items=[...(resources.data?.items??[]).map(r=>resourceItem(r,'Resources')),...(!searchCollection&&searchMode==='title'?collections.data?.items??[]:[]).map(c=>({...action('collection-'+c.id,c.name,()=>captured.openCollection?.(c),`${c.count} resources`),kind:'collection',section:'Collections'}))];
 else {
 ...
 const result=await item.onSelect();if(result.type==='push'){setStack(v=>[...v,{step:result.step,query:result.query??result.step.initialQuery??'',parentSelectedId:selected?.id??''}]);setSelectedId('');}
 ...
 const failure=((mode==='search'&&!entry)||selecting)?resources.error||(!searchCollection&&searchMode==='title'&&search.trim()?collections.error:'')||(!searchCollection&&!search.trim()?pins.error:''):'';
 const closeOrganization=()=>{setOrganization(null);onClose({restorePreview:false,restoreFocus:true});};
 const dialog=organization?.kind==='collection'?<CollectionPicker resourceIds={organization.ids} onClose={closeOrganization} onCreated={organization.ids.length?undefined:closeOrganization}/>:organization?<OrganizationManager initialKind="tag" resourceIds={organization.ids} onClose={closeOrganization} onApplied={closeOrganization} tags={[]} onTag={()=>{}} onTagRenamed={()=>{}} onCollection={()=>{}}/>:null;
 ...
 <div role="dialog" aria-modal="true" aria-label="Search and commands" className="flex flex-col w-full max-w-2xl rounded-2xl border border-[var(--border-strong)] shadow-xl overflow-hidden" style={getGlassPanelStyle(transparency,20)} onClick={e=>e.stopPropagation()} onKeyDown={e=>e.stopPropagation()}>
 ...
 <div className="flex items-center gap-2 px-4 py-3"><Search size={18}/><input ref={input} role="combobox" aria-label={entry?.step.placeholder??'Search resources'} aria-expanded="true" aria-controls="palette-results" aria-autocomplete="list" aria-activedescendant={selected?'palette-option-'+selected.id:undefined} className="w-full bg-transparent outline-none" value={currentQuery} disabled={busy} placeholder={entry?.step.placeholder??(mode==='commands'?'Find a command…':searchMode==='title'?'Search resource titles…':'Search titles and saved note text…')} onChange={e=>change(e.target.value)} onKeyDown={e=>{if(e.nativeEvent.isComposing||e.keyCode===229)return;if(e.key==='ArrowDown'||e.key==='ArrowUp'){e.preventDefault();const index=items.findIndex(i=>i.id===selected?.id);setSelectedId(items[Math.max(0,Math.min(items.length-1,index+(e.key==='ArrowDown'?1:-1)))]?.id??'');}else if(e.key==='Enter'&&selected){e.preventDefault();void execute(selected);}}}/></div>
 <footer className="order-last flex flex-wrap gap-2 items-center justify-between border-t border-border p-3 text-xs text-[var(--text-secondary)]"><span>{busy?'Working…':'Enter opens · ↑ ↓ select · Tab actions · Esc back'}</span>{selected?.resource&&<button className="library-button max-w-full min-w-0" disabled={busy} onClick={()=>void execute({id:'resource-actions',title:'Actions',onSelect:async()=>({type:'push',step:actionStep((await api.get<Resource>('/resources/'+selected.resource!.id+'/summary',{backgroundDiagnostic:true})).data)})})}><MoreHorizontal className="shrink-0" size={16}/><span className="truncate">Actions for {selected.title}</span></button>}</footer>
 <div className="px-4 pb-2 text-xs text-[var(--text-secondary)]">{searchMode==='title'?'Titles only':context.hasUnsavedChanges?'Saved content only; unsaved edits are excluded.':'Titles + saved note text'}{!entry&&mode==='search'&&<div className="flex items-center justify-between gap-2 mt-2"><span className="truncate">{searchCollection?'In '+searchCollection.name:'Entire workspace'}</span><LibraryPopover label="Search options" priority={ESCAPE_PRIORITIES.commandPalette+1}>{()=> <ResourceSearchOptions mode={searchMode} onMode={setSearchMode} collection={searchCollection} onCollection={setSearchCollection}/>}</LibraryPopover></div>}{mode==='commands'&&<p>Target: {targets.length?targets.map(r=>r.title).join(', '):'None — choose a resource explicitly'}</p>}</div>
