...
export function CollectionPicker({resourceIds,onClose,onCreated}:{resourceIds:string[];onClose:()=>void;onCreated?:(item:OrganizationItem)=>void}) {
 ...
 async function create(){if(running.current.has('create')||!query.trim())return;if(exact&&creation.current.name!==query.trim()){setErrors(v=>({...v,create:'A collection with this name already exists. Choose another name.'}));return;}running.current.add('create');setPending(v=>[...v,'create']);setErrors(v=>({...v,create:''}));const name=query.trim();if(creation.current.name!==name)creation.current={name,id:crypto.randomUUID()};
 ...
 const form=useFormKeyboard(create);
 const exact=(search===query.trim()&&Boolean(result.data?.exactMatch))||items.some(item=>item.name.toLowerCase()===query.trim().toLowerCase());
 ...
 <form {...form} className="space-y-3"><input readOnly={pending.includes('create')} autoFocus className="organization-input w-full" aria-label="Search or create collection" placeholder="Search or create a collection…" maxLength={100} value={query} onChange={e=>setQuery(e.target.value)}/>
 {(result.error||membership.error)&&<p role="alert">Could not load collections. <button type="button" onClick={()=>{result.retry();membership.retry();}}>Retry</button></p>}
 {resourceIds.length>0&&<div className="max-h-72 overflow-y-auto space-y-1">{membership.loading?<p>Loading memberships…</p>:items.map(item=><div key={item.id}><label className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-[var(--surface-hover)]"><input type="checkbox" aria-checked={item.selectedCount>0&&item.selectedCount<resourceIds.length?'mixed':item.selectedCount===resourceIds.length} checked={item.selectedCount===resourceIds.length} disabled={pending.includes(item.id)||Boolean(membership.error)} onChange={()=>void toggle(item)}/><span className="truncate flex-1">{item.name}</span>{item.selectedCount>0&&item.selectedCount<resourceIds.length&&<small>some</small>}{pending.includes(item.id)&&<small>Saving…</small>}</label>{errors[item.id]&&<p role="alert" className="text-xs text-red-500">{errors[item.id]} <button type="button" onClick={()=>void toggle(item)}>Retry</button></p>}</div>)}</div>}
 {query.trim()&&(!exact||creation.current.name===query.trim())&&<button type="submit" className="library-button w-full" disabled={pending.includes('create')}><Plus size={15}/>{pending.includes('create')?'Creating…':`Create “${query.trim()}”${resourceIds.length?' and add':''}`}</button>}
 {!resourceIds.length&&exact&&<p className="text-sm">A collection with this name already exists. Choose another name.</p>}
 {errors.create&&<p role="alert" className="text-sm text-red-500">{errors.create} <button type="button" disabled={pending.includes('create')} onClick={()=>void create()}>Retry</button></p>}
 {resourceIds.length>0&&<div className="flex gap-2 justify-end"><button type="button" className="library-button" disabled={page===0} onClick={()=>setPage(page-1)}>Previous</button><button type="button" className="library-button" disabled={!result.data||page+1>=result.data.totalPages} onClick={()=>setPage(page+1)}>Next</button></div>}
 </form></AppModal>,document.body);
}
